...
// console.log("this is the current dir",__dirname1)

// if (process.env.NODE_ENV=== "production"){

//   app.use(express.static(path.join(__dirname1 , "/frontend/z_front/build")));
...
//   app.get("*", (req,res)=>{
//     res.sendFile(path.resolve(__dirname1 ,"frontend" , "z_front" , "build" , "index.html"))
//   })
...
app.get("/filter/:filter_query", async(req, res) => {
  ...
  //if query is for rating
    ...
    const resultant_products = await Food_item.find({"rating" : { $gte: f_query } })
...
app.get("/restaurant/:searched_item", async (req, res) => {
  ...
  if (find_restaurant == null) {
    ...
    res.send('<div>The restaurant with name is not listed on our ZOMATOO</div>');
  } 
  else {
